...
class SchedulerDuling {
  static void duling(QueueList<Processo> filaPronto) {
    Dispatcher dispatcher = new Dispatcher(filaPronto);

    int tempoAtual = 0;

    while (dispatcher.fila.isNotEmpty) {
      Processo p = dispatcher.getProcessDuling(tempoAtual);
      ...
      for (int i = p.tempoCPU; i <= p.tempoRajada; i++) {
        p = dispatcher.getMenorPrioridade(p, tempoAtual, i);
        if (i == 0) {
